...
mod trees {
    ...
    macro_rules! table {
        ($builder:expr; BN($pos:literal, $left:literal, $right:literal) $($rest:tt)*) => {
            $builder.push(HuffmanTableEntry::Branch($left, $right));
            table!($builder; $($rest)*);
        };
        ($builder:expr; LN($pos:literal, $value:expr) $($rest:tt)*) => {
            $builder.push(HuffmanTableEntry::Leaf($value));
            table!($builder; $($rest)*);
        };
        ($builder:expr;) => {};
    }

    pub static LENGTH_TREE: LazyLock<HuffmanTable<u8>> = std::sync::LazyLock::new(|| {
        ...
        HuffmanTable { entries: builder }
    });

    pub static DISTANCE_TREE: LazyLock<HuffmanTable<u8>> = std::sync::LazyLock::new(|| {
        let mut builder = Vec::new();
        table!(builder;
            BN(0, 1, 2)
            BN(1, 3, 4)       BN(2, 5, 6)
    ...
    pub static ASCII_TREE: LazyLock<HuffmanTable<u8>> = std::sync::LazyLock::new(|| {
        let mut builder = Vec::new();
        table!(builder;
