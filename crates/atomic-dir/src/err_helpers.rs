...
#[error("{context}: {error}")]
pub(crate) struct ErrorWithContext<T, E> {
    context: T,
    #[source]
    error: E,
}
...
where
    ...
    E: std::error::Error,
{
    pub(crate) fn new(context: T, error: E) -> Self {
        Self { context, error }
    }
}
...
macro_rules! io_err_map {
    ($kind:ident, $fmt:literal $($arg:tt)*) => {
        |e| std::io::Error::new(std::io::ErrorKind::$kind, $crate::err_helpers::ErrorWithContext::new(format!($fmt $($arg)*), e))
    };
    ($kind:ident) => {
