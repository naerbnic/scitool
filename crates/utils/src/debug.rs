...
pub fn hex_dump(data: &[u8], initial_offset: usize) {
    ...
    let offset_padding = " ".repeat(num_offset_hex_chars as usize);
    println!(
        "{} 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F",
        offset_padding
    );
    ...
    while !remaining_data.is_empty() {
        ...
        let line_end = std::cmp::max(remaining_data.len(), 16 - line_start) + line_start;
        let line_length = line_end - line_start;

        let empty_prefix = "   ".repeat(line_start);
        ...
        println!("{} {}{}", offset_text, empty_prefix, line_hex);
